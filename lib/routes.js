...
async function handleScan(req, res, deps) {
  ...
  const { text } = isPlaintext(file)
    ? { text: readFileSync(file, 'utf8') }
    : { text: decodeFull(readFileSync(file)) };
...
async function handleRepair(req, res, deps) {
  ...
  const { text, plaintext } = isPlaintext(file)
    ? { text: readFileSync(file, 'utf8'), plaintext: true }
    : { text: decodeFull(readFileSync(file)), plaintext: false };
  ...
  // 离线迁移链校验（目标实例自带 catalog；找不到则 skipped）
  let validation = null;
  try {
    validation = await validateMigrationChain(repaired.text, { dshHome: deps.dshHome, installRoot: deps.installRoot });
  } catch (err) {
    validation = { ok: false, error: String(err?.message ?? err) };
  }
  ...
  response.backup = backupDir;
  response.encodedBytes = encoded.length;
  sendJson(res, 200, response);
}
