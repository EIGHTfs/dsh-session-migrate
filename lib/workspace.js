...
/**
 * 解码目录名主体：`~XXXX` 转 Unicode 码点，`-` 转 `/`。
 *
 * @param {string} body - `--` 之间的内容。
 * @returns {string}
 */
function decodeBody(body) {
  ...
  for (let i = 0; i < body.length; i++) {
    const ch = body[i];
    // `~XXXX` 是转义：四个十六进制 → 一个 Unicode 码点
    const cp = decodeEscape(body, i);
    if (cp !== null) {
      out += cp;
      i += 4;
      continue;
...
/**
 * 尝试把 `body[i]` 处的 `~XXXX` 转成 Unicode 字符。
 *
 * @param {string} body - 目录名主体。
 * @param {number} i - `~` 的位置。
 * @returns {string|null} 转义成功返回码点字符，否则 null。
 */
function decodeEscape(body, i) {
  if (body[i] !== '~' || i + 4 >= body.length) return null;
  const hex = body.slice(i + 1, i + 5);
  return /^[0-9A-Fa-f]{4}$/.test(hex) ? String.fromCodePoint(parseInt(hex, 16)) : null;
}
