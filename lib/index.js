...
import { readHeaderAny, decodeFull, isPlaintext, textToZstd } from './engine/zstd.js';
...
/**
 * 把内容缺陷扫描结果整理成多行文本（供 session_migrate_check 的内容段 / scan 工具输出）。
 *
 * @param {object} scan - scanLog 的结果。
 * @returns {string}
 */
function formatScanResult(scan) {
  const lines = [`内容缺陷: ${scan.blocking} 个阻塞迁移`];
  ...
  if (scan.replacementCharacters > 0) lines.push(`  替换字符(U+FFFD): ${scan.replacementCharacters}（可能被早期工具写坏）`);
  if (scan.blocking > 0) {
    lines.push('  → 用 session_migrate_repair 修复（只读 scan 见 CLI: node cli.mjs scan <文件>）');
  }
  return lines.join('\n');
}
  ...
  return [
    {
      ...
      execute: async ({ path }) => {
        ...
        const out = [formatCheckResult(path, r)];
        // 附带内容缺陷扫描（首帧通过时；坏行/异常不打断体检本身）
        try {
          const { text } = isPlaintext(path)
            ? { text: readFileSync(path, 'utf8') }
            : { text: decodeFull(readFileSync(path)) };
          const scan = scanLog(parseLog(text).rows, { text });
          if (scan.blocking > 0 || scan.replacementCharacters > 0) out.push('', formatScanResult(scan));
        } catch {
          /* 内容扫描失败不阻塞体检结论 */
        }
        return out.join('\n');
    ...
    {
      name: 'session_migrate_repair',
      description:
        '修复一个无法迁移的旧版（v0）会话文件的内容缺陷：诊断（tool-call-chunks/消息块/block-end 的空 id/name、'
        + 'tool/call 的 arguments 不一致——都是旧版写入器留下的空字段，会让 DSH 迁移链拒绝、打开报 history unavailable）'
        + '→ 备份原文件 → 只重写变化的行 → （可选）用目标实例自带的格式目录离线跑完整 v0→v1→v2→v3 迁移链校验 → 落盘。'
        + '幂等：无缺陷时返回无需修复。路径可直接传会话文件（zstd 或明文 jsonl）。',
      parameters: normalizeParameters({ path: 'string', home: 'string?' }),
      render: textRender,
      execute: async ({ path, home }) => {
        if (!path) throw new Error('需要 path');
        const paths = resolvePaths(config);
        const dshHome = (home && existsSync(home)) ? home : paths.dshHome;
        const { text, plaintext } = isPlaintext(path)
          ? { text: readFileSync(path, 'utf8'), plaintext: true }
          : { text: decodeFull(readFileSync(path)), plaintext: false };
        const before = scanLog(parseLog(text).rows, { text });
        const repaired = repairLogText(text);

        if (repaired.changedLines.length === 0) {
          return '无需修复：未发现本工具可处理的缺陷。\n' + formatScanResult(before);
        }

        const lines = [
          `待修复行数: ${repaired.changedLines.length}`,
          `修复统计: ${JSON.stringify(repaired.stats)}`,
        ];
        if (repaired.skipped.length) {
          lines.push(`无法自动修复 ${repaired.skipped.length} 处（缺 delta 参考）: ${repaired.skipped.map((s) => 'L' + s.line).join(', ')}`);
        }

        // 离线迁移链校验（目标实例自带 catalog；找不到则提示跳过）
        let validation = null;
        try {
          validation = await validateMigrationChain(repaired.text, { dshHome, installRoot: paths.installRoot });
        } catch (e) {
          validation = { ok: false, error: String(e?.message ?? e) };
        }
        if (validation?.skipped) lines.push(`迁移链校验: 跳过（${validation.skipped}）`);
        else if (validation) lines.push(`迁移链校验: ${validation.ok ? `通过 ✅（${validation.events} 个 v3 事件）` : `失败 ❌ ${validation.error}`}`);

        // 校验失败（且 catalog 可用）→ 拒绝写入
        if (validation && validation.ok === false && validation.skipped === undefined) {
          lines.push('拒绝写入：修复后仍无法通过迁移链校验。');
          return lines.join('\n');
        }

        // 备份 → 落盘（原子写）
        const backupDir = join(dirname(path), `${basename(path)}.repair-bak-${Date.now()}`);
        ...
        lines.push(`已备份 → ${backupDir}/`);
        lines.push(`已落盘 → ${path}（${plaintext ? '明文' : 'zstd'}）`);
        return lines.join('\n');
      },
    },
    {
