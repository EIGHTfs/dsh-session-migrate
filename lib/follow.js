...
export function findToken(dshHome) {
  ...
  cands.push(join(root, 'dsh-proxy.log'));
  ...
  for (const p of cands) {
    ...
    if (!p.includes('proxy')) return { token, file: p, port };
    if (!fallback.token) fallback = { token, file: p, port };
  }
  return fallback;
}
...
export async function exchangeCookie(port, token) {
  try {
    const res = await httpCall({ port, path: `/?token=${encodeURIComponent(token)}` });
...
function parseWsUrl(url) {
  ...
  if (!m) throw new Error(`无法解析 WebSocket 地址: ${url}`);
...
function drainFrames(st, io) {
  for (;;) {
    ...
    if (len === LEN_16BIT) {
      if (st.buffer.length < 4) return;
      len = st.buffer.readUInt16BE(2);
      offset = 4;
    } else if (len === LEN_64BIT) {
      if (st.buffer.length < 10) return;
      len = Number(st.buffer.readBigUInt64BE(2));
      offset = 10;
    }
    if (masked) offset += 4;
    if (st.buffer.length < offset + len) return;

    const payload = st.buffer.subarray(offset, offset + len);
    st.buffer = st.buffer.subarray(offset + len);
    ...
    if (opcode === OP_TEXT || opcode === OP_BINARY) {
      ...
      if (opcode === OP_TEXT) {
        try { io.onText?.(payload.toString('utf8')); } catch { /* 同上 */ }
      }
    }
    // pong 等其他帧无需处理
  }
...
export function openWebSocket(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (!closed) { closed = true; try { socket.destroy(); } catch { /* 已关 */ } }
    ...
    socket.on('data', (chunk) => {
      ...
      if (!handshakeDone) {
        ...
        handshakeDone = true;
        clearTimeout(timer);
        const err = verifyHandshake(head, expect);
        if (err) { fail(err); return; }
        resolve({ close, socket });
      }
    ...
    socket.on('error', (err) => {
      if (!handshakeDone) { clearTimeout(timer); reject(err); return; }
      closed = true;
    });
    socket.on('close', () => {
      closed = true;
...
export async function followSession(opts) {
  ...
  const port = opts.port || found.port || 30801;
  ...
  return new Promise((resolve) => {
    ...
    openWebSocket({
      ...
      onText: (text) => {
        ...
        if (msg.type === 'item') {
          ...
          if (value && value.type === 'snapshot') {
            done({ ok: true, snapshot: value });
          } else if (value && value.type === 'error') {
            done({ ok: false, error: value.message ?? JSON.stringify(value).slice(0, 300) });
          }
      ...
      wsRef = ws;
      // ⚠️ streamId 必须是**字符串**：用数字（如 1）服务端不会回任何帧，
      //    表现为「静默超时」；用字符串才会收到 item/end（实测）。
      ...
      ws.socket.write(encodeFrame(0x1, frame));
    }).catch((err) => {
