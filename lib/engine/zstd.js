...
/**
 * 全量解码：把多帧 zstd 流的每一帧解压后拼接为文本。
 *
 * 为什么不用 zstdDecompressSync(buf) 直接解：Node 的 zstd 助手只解**第一帧**
 * （与 clone 的 zstd.mjs 注释一致），而 DSH 日志是追加式多帧流，必须逐帧解。
 * 帧边界 = 相邻两个魔数之间（本项目的 v0 布局为每行独立一帧；即使修复/校验
 * 场景遇到其它分帧策略，魔数定位依然成立——zstd 帧起点固定以魔数开始）。
 *
 * @param {Buffer} buf 完整 zstd 会话文件
 * @returns {string} 拼接后的明文（UTF-8 一次解码，避免分块切断多字节字符）
 */
export function decodeFull(buf) {
  const starts = [];
  let pos = 0;
  for (;;) {
    const i = buf.indexOf(ZSTD_MAGIC, pos);
    if (i < 0) break;
    starts.push(i);
    pos = i + ZSTD_MAGIC.length;
  }
  if (starts.length === 0) throw new Error('不是 zstd 流：未找到帧魔数');
  const parts = [];
...
/**
 * 明文 JSONL → zstd Buffer（内存版，修复/校验落盘用）。
 * 与 plaintextToZstd 同一分帧策略：首行单独一帧（header），其余每行各一帧。
 * @param {string} text 明文日志（多行 JSONL）
 * @returns {Buffer}
 */
export function textToZstd(text) {
  const lines = text.split('\n');
  if (lines.length && lines[lines.length - 1] === '') lines.pop();
  if (!lines.length) throw new Error('空文件');
  const frames = [frameLine(lines[0])];
