...
/** 正文行高（CSS line-height 单位倍数）。 */
const CSS_LINE_HEIGHT = 1.5
/** 标题字重（CSS font-weight）。 */
const CSS_TITLE_WEIGHT = 600

/** 页面样式（前缀 dshsm_ 避免与其它插件冲突）。 */
const cssText = `
.dshsm_root{display:flex;flex-direction:column;gap:14px;font-size:13px;line-height:${CSS_LINE_HEIGHT}}
...
.dshsm_cardTitle{font-size:13px;font-weight:${CSS_TITLE_WEIGHT};margin:0 0 8px}
...
function registerSettingsSection(ctx, React) {
  ...
  slots.inject('settings.section', () => injectSection(slots, section, React))
}

/**
 * 把分区注册进 slots 服务（独立函数避免 inject 回调内再嵌回调）。
 *
 * @param {object} slots - 宿主 slots 服务。
 * @param {object} section - 分区定义。
 * @param {Function} React - 宿主注入的 React。
 * @returns {void}
 */
function injectSection(slots, section, React) {
  slots.register(section, () => React.createElement(MigratePage, null))
}
